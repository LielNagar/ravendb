...
internal static class CertificateLoaderUtil
{
    ...
    {
        return CreateCertificate(f => new X509Certificate2(rawData, password == "" ? null:password, f), flags);
    }

    internal static X509Certificate2 CreateCertificate(string fileName, string password = null, X509KeyStorageFlags? flags = null)
    {
        return CreateCertificate(f => new X509Certificate2(fileName, password == "" ? null:password, f), flags);
    }
